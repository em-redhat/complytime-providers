...
- [ ] 1.1 Create `internal/archive/archive.go` with exported `ResolveComplypackPath(contentPath string) (string, error)`, `ExtractTarGz(archive, dst string) error`, and unexported `writeFileFromTar(path string, r io.Reader) (int64, error)` (returns bytes written)
...
- [ ] 1.3 Add aggregate `totalBytes` counter in `ExtractTarGz` loop, updated after each `writeFileFromTar` call; abort with descriptive error when exceeded
- [ ] 1.4 Add `fileCount` counter in `ExtractTarGz` loop, checked before each regular file extraction; abort with descriptive error when exceeded
...
- [ ] 2.1 Create `internal/archive/archive_test.go` with test helpers (reuse ampel's `createTarGz` + `tarEntry` pattern)
- [ ] 2.2 Migrate existing tests: directory passthrough, tar.gz extraction, idempotent skip, path traversal rejection, absolute path rejection, symlink rejection, hard link rejection, oversized file rejection, non-existent path, corrupt archive, dot directory entry, file permissions
- [ ] 2.3 Add `TestExtractTarGz_AggregateSizeExceeded` — archive with multiple files whose total exceeds 500 MB
- [ ] 2.4 Add `TestExtractTarGz_FileCountExceeded` — archive with more than 10,000 regular file entries
- [ ] 2.5 Add `TestExtractTarGz_AggregateSizeWithinLimit` — archive with total just under the limit succeeds
- [ ] 2.6 Add `TestExtractTarGz_FileCountWithinLimit` — archive with count just under the limit succeeds
- [ ] 2.7 Add `TestResolveComplypackPath_FailedExtractionCleansUp` — verify partial extraction is removed on error
...
- [ ] 4.2 Update `cmd/opa-provider/server/server.go` to import `internal/archive` and call `archive.ResolveComplypackPath` and `archive.ExtractTarGz` where needed
...
- [ ] 6.2 Update AGENTS.md project structure to include `internal/archive/` under the internal/ section
