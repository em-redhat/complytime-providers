...
**Migration**: No action required. complyctl no longer calls Export on any provider. Remove any local tooling that depends on `SupportsExport: true` in the AMPEL provider's Describe response.
...
**Migration**: No action required. complyctl no longer invokes the Export RPC. The result-to-GemaraEvidence conversion logic is deleted along with the ProofWatch integration. When export is redesigned upstream, a new implementation will be created from scratch.
